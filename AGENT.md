...
Read this file first. Then `PIPELINE.md` (technical spec), `STYLE-RULES.md`
(hard-won rules that override everything), and the style pack you're using in
`styles/`.
...
```bash
python3 tools/example_build_beats.py      # writes src/beats/<slug>.json
...
Extract a frame strip across all beats and review it **as a hostile viewer**:
